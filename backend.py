...
class AppSession(ApplicationSession):
    ...
    def __init__(self, appConfig=None):
        ApplicationSession.__init__(self, appConfig)
        self.db = common.connect_db()
    ...
    def onJoin(self, details):
        ...
        try:
            yield self.register(self.check_username, PUBLIC_PREFIX + 'checkusername')
            yield self.register(self.register, PUBLIC_PREFIX + 'register')
            yield self.register(self.login, PUBLIC_PREFIX + 'login')
            self.log.info("Registered methods to {prefix}", prefix=PUBLIC_PREFIX)
            yield self.register(self.save, PUBLIC_PREFIX + 'save')
            yield self.register(self.load, PUBLIC_PREFIX + 'load')
            yield self.register(self.delete, PUBLIC_PREFIX + 'delete')
            yield self.register(self.list_campaigns, PUBLIC_PREFIX + 'list')
            yield self.register(self.add_editor, PUBLIC_PREFIX + 'add_editor')
            yield self.register(self.remove_editor, PUBLIC_PREFIX + 'remove_editor')
            yield self.register(self.list_editors, PUBLIC_PREFIX + 'list_editors')

        except Exception as e:
            self.log.error("Unexpected exception while registering public RPC's {e}", e=str(e))
    ...
    def register(self, username, raw_password):
        """
    ...
    def save(self, name, data, campaignid=None):
        """
        ...
        try:
            if campaignid is not None:
                ...
                cursor.execute("insert into kdm (owner_id, name, content) values (?, ?, ?)", (self.user_id, name,
                                                                                               data))
                campaignid = cursor.lastrowid
    ...
    def delete(self, campaignid):
        ...
        if not self.is_logged_in():
            raise Exception("You are not logged in!")
        self.db.execute("delete from kdm where owner_id=? and kdm_id=?", (self.user_id, campaignid))
        self.db.commit()
    ...
    def add_editor(self, campaignid, username):
        """
    ...
    def remove_editor(self, campaignid, username):
        try:
            if not self.is_logged_in():
                raise Exception("You are not logged in!")
            results = self.db.execute("select user_id from users where username=?", (username,)).fetchone()
    ...
    def list_editors(self, campaignid):
        if not self.is_logged_in():
            raise Exception("You are not logged in!")
    ...
    def list_joinable(self):
        return self.db.execute("SELECT users.username, kdm.name from kdm_editors inner join users on "
                        "kdm_editors.user_id=users.user_id inner join kdm on kdm_editors.kdm_id=kdm.kdm_id "
                        "where users.user_id=?", (self.user_id,)).fetchall()

    def join_session(self, campaignid):
        if not self.is_logged_in():
            raise Exception("You are not logged in!")
        result = self.db.execute("select exists(select 1 from kdm_editors where user_id=? and kdm_id=?",
                                 (self.user_id, campaignid)).fetchone()
        if result == 0:
            raise Exception("You are not authorised to edit that campaign")
